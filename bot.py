        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(url) as response:
                if response.status == 200:
                    ...
                    # Проверяем статус username
                    status = data.get("status", "")
                    # Если username не продается и статус available/free - он свободен
                    if not data.get("selling") and status in ["available", "free", "unavailable", ""]:
                        return True
                    return False
...
async def menu_search_callback(callback: types.CallbackQuery, state: FSMContext):
    """Поиск username из главного меню"""
