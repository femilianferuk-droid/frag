...
# Премиум эмодзи ID (ТОЛЬКО ПРЕМИУМ)
EMOJI = {
...
def get_settings_keyboard() -> types.InlineKeyboardMarkup:
    ...
    builder.row(types.InlineKeyboardButton(
        ...
        callback_data="settings_change_welcome",
        style="primary",
        icon_custom_emoji_id=EMOJI["write"]
    ))
    builder.row(types.InlineKeyboardButton(
...
async def check_username_availability(username: str) -> bool:
    """Реальная проверка доступности username через Fragment API"""
    try:
        async with aiohttp.ClientSession() as session:
            url = f"{FRAGMENT_API}/username/{username}"
            
            async with session.get(url) as response:
                if response.status == 200:
                    data = await response.json()
                    # Если username свободен, status будет "available" или "free"
                    # Если занят - будет информация о владельце и цене
                    status = data.get("status", "")
                    if status in ["available", "free", "unavailable"] and not data.get("selling"):
                        return True
                    return False
                elif response.status == 404:
                    # 404 может означать что username свободен
                    return True
                return False
    except Exception as e:
        logger.error(f"Ошибка при проверке username {username}: {e}")
        return False


def generate_usernames(length: int, keywords: List[str] = None, count: int = 100) -> List[str]:
    ...
    if keywords:
        # Генерируем username с ключевыми словами
        for keyword in keywords:
            ...
            # Добавляем случайные буквы до и после ключевого слова
        ...
        # Генерируем случайные username заданной длины
...
async def search_free_usernames(length: int, keywords: List[str] = None, max_results: int = 10) -> List[str]:
    """Поиск свободных username через Fragment"""
    free_usernames = []
    checked = set()
    
    # Генерируем username для проверки
    usernames_to_check = generate_usernames(length, keywords, count=50)
    
    # Создаем очередь для параллельной проверки
    tasks = []
    
    for username in usernames_to_check:
        if username not in checked:
            checked.add(username)
            tasks.append(check_username_availability(username))
            
            # Если набрали достаточно результатов, останавливаемся
    ...
    for username, is_free in zip(usernames_to_check[:len(results)], results):
...
def format_username_link(username: str) -> str:
    """Форматирование username как ссылки"""
...
@dp.message(F.text == "Поиск username")
async def search_username_handler(message: types.Message, state: FSMContext):
    """Начало поиска username"""
    await message.answer(
...
async def process_length_selection(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f'<b><tg-emoji emoji-id="{EMOJI["write"]}">✍</tg-emoji> Введите ключевые слова через пробел (или отправьте "0" для случайного поиска):</b>',
        parse_mode="HTML",
...
async def process_keywords(message: types.Message, state: FSMContext):
    ...
    if free_usernames:
        ...
        for i, username in enumerate(free_usernames, 1):
            fragment_link = format_username_link(username)
            result_text += f'<b>{i}.</b> <a href="{fragment_link}">@{username}</a>\n'
        
        result_text += f'\n<b><tg-emoji emoji-id="{EMOJI["info"]}">ℹ</tg-emoji> Нажмите на username чтобы перейти на Fragment</b>'
    else:
    ...
    # Создаем клавиатуру с кнопками для каждого username
    if free_usernames:
        builder = InlineKeyboardBuilder()
        for i, username in enumerate(free_usernames[:5], 1):
            fragment_link = format_username_link(username)
            builder.row(types.InlineKeyboardButton(
                text=f"@{username}",
                url=fragment_link,
                style="primary" if i % 2 == 1 else "success",
...
async def change_name_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        '<b>Отправьте новое название бота:</b>',
        parse_mode="HTML",
        reply_markup=get_back_keyboard()
    )
...
async def process_bot_name(message: types.Message, state: FSMContext):
    """Обработка нового названия бота"""
    if not is_admin(message.from_user.id):
        return
    ...
    await message.answer(
        f'<b><tg-emoji emoji-id="{EMOJI["check"]}">✅</tg-emoji> Название бота изменено на:</b> {new_name}',
        parse_mode="HTML",
        reply_markup=get_admin_panel_keyboard()
    )
    await state.clear()
...
async def process_welcome_message(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        f'<b><tg-emoji emoji-id="{EMOJI["check"]}">✅</tg-emoji> Приветственное сообщение изменено на:</b>\n\n{new_welcome}',
        parse_mode="HTML",
        reply_markup=get_admin_panel_keyboard()
    )
    await state.clear()
...
async def add_media_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f'<b><tg-emoji emoji-id="{EMOJI["media"]}">🖼</tg-emoji> Отправьте категорию для медиа (например: "фон", "аватар"):</b>',
        parse_mode="HTML",
        reply_markup=get_back_keyboard()
    )
...
async def process_media_category(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        f'<b><tg-emoji emoji-id="{EMOJI["media"]}">🖼</tg-emoji> Отправьте медиа-файл для категории "{category}":</b>',
        parse_mode="HTML",
        reply_markup=get_back_keyboard()
    )
...
async def process_media_file(message: types.Message, state: FSMContext):
    ...
    if message.photo:
        ...
        await message.answer(
            f'<b><tg-emoji emoji-id="{EMOJI["check"]}">✅</tg-emoji> Медиа добавлено в категорию "{category}"</b>',
            parse_mode="HTML",
            reply_markup=get_admin_panel_keyboard()
        )
    else:
...
async def info_handler(message: types.Message):
    """Информация о боте"""
    info_text = f'''
<b><tg-emoji emoji-id="{EMOJI["info"]}">ℹ</tg-emoji> О боте</b>

<b>Название:</b> {bot_settings["bot_name"]}
<b>Функции:</b>
<tg-emoji emoji-id="{EMOJI["search"]}">🔍</tg-emoji> Поиск свободных username
<tg-emoji emoji-id="{EMOJI["tag"]}">🏷</tg-emoji> Поиск по ключевым словам
<tg-emoji emoji-id="{EMOJI["link"]}">🔗</tg-emoji> Прямые ссылки на Fragment

<b>Поиск работает через официальный API Fragment.com</b>
'''
